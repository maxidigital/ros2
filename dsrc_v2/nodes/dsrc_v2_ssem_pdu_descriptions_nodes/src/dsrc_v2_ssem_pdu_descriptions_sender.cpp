 ...
 *
 */
#include <dsrc_v2_ssem_pdu_descriptions_sender.h>
 ...
 */
void
wind::wind_ros::Sender_dsrc_v2_ssem_pdu_descriptions::start()
{
    encoder_ = new wind::encoder_dsrc_v2_ssem_pdu_descriptions::WerEncoder(slogger_, debug_);
	...
	this->udp_sender_ = new wind::comm::UDPSender(output_ip_, port.str());
		...
		this->subscriber_ = node_handle_.subscribe(topic_, 10, &wind::wind_ros::Sender_dsrc_v2_ssem_pdu_descriptions::callback, this);
	#else		
		auto callback1 = [this](const dsrc_v2_ssem_pdu_descriptions_msgs::msg::SSEM::SharedPtr msg) {this->callback(msg);};
		this->subscriber = this->create_subscription<dsrc_v2_ssem_pdu_descriptions_msgs::msg::SSEM>(topic_, 10, callback1);
    #endif
 ...
 */
void
wind::wind_ros::Sender_dsrc_v2_ssem_pdu_descriptions::callback(
#if WIND_ROS_VERSION == 1
  const dsrc_v2_ssem_pdu_descriptions_msgs::SSEM::ConstPtr& msg)
#else		
  const std::shared_ptr<dsrc_v2_ssem_pdu_descriptions_msgs::msg::SSEM>& msg)
#endif
{	
	if(busy_)
		return;
    ...
    slogger_->print() << "Received SSEM message on '" << tools::green(topic_) << "' topic";
    ...
    //slogger_->print() << "SSEM REQUEST messages sent: success(" << ++messages_counter_ << "/" << failed_ << ")";
...
void
wind::wind_ros::Sender_dsrc_v2_ssem_pdu_descriptions::print_message(std::size_t headerLen, std::size_t messageLen) {
	...
	slogger_->print() << "Sending message: name(SSEM) header(REQUEST) " << _host_.str() << " " << _len_.str() << _success_;	
}
