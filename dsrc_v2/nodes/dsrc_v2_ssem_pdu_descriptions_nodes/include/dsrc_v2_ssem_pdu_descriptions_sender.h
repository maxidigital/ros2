 ...
 *
 */
#ifndef DSRC_V2_SSEM_PDU_DESCRIPTIONS_SENDER_H
#define DSRC_V2_SSEM_PDU_DESCRIPTIONS_SENDER_H
...
#if WIND_ROS_VERSION == 1
  class Sender_dsrc_v2_ssem_pdu_descriptions
#else
  class Sender_dsrc_v2_ssem_pdu_descriptions : public rclcpp::Node
#endif
{
...
public:
	...
	#if WIND_ROS_VERSION == 1
     Sender_dsrc_v2_ssem_pdu_descriptions() {	
	#else
     Sender_dsrc_v2_ssem_pdu_descriptions() : rclcpp::Node(SENDER_NODE_NAME) {
    #endif		
		messages_counter_ = 0;
		failed_ = 0;
...
private:
    ...
    const int              INDICATION_HEADER_SIZE_ = 52;
	 ...
	 void callback(const dsrc_v2_ssem_pdu_descriptions_msgs::SSEM::ConstPtr& msg);	 
	#else
	 rclcpp::Subscription<dsrc_v2_ssem_pdu_descriptions_msgs::msg::SSEM>::SharedPtr subscriber;
     void callback(const std::shared_ptr<dsrc_v2_ssem_pdu_descriptions_msgs::msg::SSEM>& msg);	 
    #endif
	
    unsigned short int     btp_port_ = 2008;
    std::string            name_;    
	std::string            topic_;
	...
	wind::encoder_dsrc_v2_ssem_pdu_descriptions::WerEncoder* encoder_;
...
#endif //DSRC_V2_SSEM_PDU_DESCRIPTIONS_SENDER_H