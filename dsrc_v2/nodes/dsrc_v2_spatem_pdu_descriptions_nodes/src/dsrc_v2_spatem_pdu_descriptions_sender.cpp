 ...
 *
 */
#include <dsrc_v2_spatem_pdu_descriptions_sender.h>
 ...
 */
void
wind::wind_ros::Sender_dsrc_v2_spatem_pdu_descriptions::start()
{
    encoder_ = new wind::encoder_dsrc_v2_spatem_pdu_descriptions::WerEncoder(slogger_, debug_);
	...
	this->udp_sender_ = new wind::comm::UDPSender(output_ip_, port.str());
		...
		this->subscriber_ = node_handle_.subscribe(topic_, 10, &wind::wind_ros::Sender_dsrc_v2_spatem_pdu_descriptions::callback, this);
	#else		
		auto callback1 = [this](const dsrc_v2_spatem_pdu_descriptions_msgs::msg::SPATEM::SharedPtr msg) {this->callback(msg);};
		this->subscriber = this->create_subscription<dsrc_v2_spatem_pdu_descriptions_msgs::msg::SPATEM>(topic_, 10, callback1);
    #endif
 ...
 */
void
wind::wind_ros::Sender_dsrc_v2_spatem_pdu_descriptions::callback(
#if WIND_ROS_VERSION == 1
  const dsrc_v2_spatem_pdu_descriptions_msgs::SPATEM::ConstPtr& msg)
#else		
  const std::shared_ptr<dsrc_v2_spatem_pdu_descriptions_msgs::msg::SPATEM>& msg)
#endif
{	
	if(busy_)
		return;
    ...
    slogger_->print() << "Received SPATEM message on '" << tools::green(topic_) << "' topic";
    ...
    //slogger_->print() << "SPATEM REQUEST messages sent: success(" << ++messages_counter_ << "/" << failed_ << ")";
...
void
wind::wind_ros::Sender_dsrc_v2_spatem_pdu_descriptions::print_message(std::size_t headerLen, std::size_t messageLen) {
	...
	slogger_->print() << "Sending message: name(SPATEM) header(REQUEST) " << _host_.str() << " " << _len_.str() << _success_;	
}
