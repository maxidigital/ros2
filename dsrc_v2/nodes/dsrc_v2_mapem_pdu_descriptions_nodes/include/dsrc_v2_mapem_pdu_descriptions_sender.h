...
#else
  class Sender_dsrc_v2_mapem_pdu_descriptions : public rclcpp::Node
#endif
{
...
public:
	...
	#else
     Sender_dsrc_v2_mapem_pdu_descriptions() : rclcpp::Node(SENDER_NODE_NAME) {
    #endif		
		messages_counter_ = 0;
		failed_ = 0;
...
private:
    ...
    const int              INDICATION_HEADER_SIZE_ = 52;
	 ...
	 void callback(const dsrc_v2_mapem_pdu_descriptions_msgs::MAPEM::ConstPtr& msg);	 
	#else
	 rclcpp::Subscription<dsrc_v2_mapem_pdu_descriptions_msgs::msg::MAPEM>::SharedPtr subscriber;
     void callback(const std::shared_ptr<dsrc_v2_mapem_pdu_descriptions_msgs::msg::MAPEM>& msg);	 
    #endif
	
    unsigned short int     btp_port_ = 2003;
    std::string            name_;    
	std::string            topic_;
	...
	wind::encoder_dsrc_v2_mapem_pdu_descriptions::WerEncoder* encoder_;
