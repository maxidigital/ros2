 ...
 *
 */
#include <dsrc_v2_mapem_pdu_descriptions_sender.h>
 ...
 */
void
wind::wind_ros::Sender_dsrc_v2_mapem_pdu_descriptions::start()
{
    encoder_ = new wind::encoder_dsrc_v2_mapem_pdu_descriptions::WerEncoder(slogger_, debug_);
	...
	this->udp_sender_ = new wind::comm::UDPSender(output_ip_, port.str());
		...
		this->subscriber_ = node_handle_.subscribe(topic_, 10, &wind::wind_ros::Sender_dsrc_v2_mapem_pdu_descriptions::callback, this);
	#else		
		auto callback1 = [this](const dsrc_v2_mapem_pdu_descriptions_msgs::msg::MAPEM::SharedPtr msg) {this->callback(msg);};
		this->subscriber = this->create_subscription<dsrc_v2_mapem_pdu_descriptions_msgs::msg::MAPEM>(topic_, 10, callback1);
    #endif
 ...
 */
void
wind::wind_ros::Sender_dsrc_v2_mapem_pdu_descriptions::callback(
#if WIND_ROS_VERSION == 1
  const dsrc_v2_mapem_pdu_descriptions_msgs::MAPEM::ConstPtr& msg)
#else		
  const std::shared_ptr<dsrc_v2_mapem_pdu_descriptions_msgs::msg::MAPEM>& msg)
#endif
{	
	if(busy_)
		return;
    ...
    slogger_->print() << "MAPEM message received on topic '" << tools::green(topic_) << "'";
    ...
    //slogger_->print() << "MAPEM REQUEST messages sent: success(" << ++messages_counter_ << "/" << failed_ << ")";
...
void
wind::wind_ros::Sender_dsrc_v2_mapem_pdu_descriptions::print_message(std::size_t headerLen, std::size_t messageLen) {
	...
	slogger_->print() << "Sending message: name(MAPEM) header(REQUEST) " << _host_.str() << " " << _len_.str() << _success_;	
}
