 ...
 *
 */
#ifndef DSRC_V2_IVIM_PDU_DESCRIPTIONS_RECEIVER_H
#define DSRC_V2_IVIM_PDU_DESCRIPTIONS_RECEIVER_H
...
{
	#if WIND_ROS_VERSION == 1
      class Receiver_dsrc_v2_ivim_pdu_descriptions
	#else
	  class Receiver_dsrc_v2_ivim_pdu_descriptions : public rclcpp::Node
    #endif
    {
    public:
		 ...
		 */
        #if WIND_ROS_VERSION == 1
		Receiver_dsrc_v2_ivim_pdu_descriptions() : node_handle_("~"),
			publisher_(nullptr),
			...
			busy_(false)
		#else
		Receiver_dsrc_v2_ivim_pdu_descriptions() : Node("receiver_dsrc_v2_ivim_pdu_descriptions"),
			messages_counter_(0),
			slogger_(nullptr),
			debug_(false),
			busy_(false)
		 ...
		 * Destructor
		 */
        ~Receiver_dsrc_v2_ivim_pdu_descriptions() {
			#if WIND_ROS_VERSION == 1
            if (publisher_) {
                delete publisher_;
                publisher_ = nullptr;
    ...
    private:        	
		...
		#else
		  rclcpp::Publisher<dsrc_v2_ivim_pdu_descriptions_msgs::msg::IVIM>::SharedPtr publisher_;
          rclcpp::Publisher<dsrc_v2_ivim_pdu_descriptions_msgs::msg::IVIM>::SharedPtr mirror_publisher_;
		#endif
		...
		bool            debug_;
		bool            busy_;
		wind::decoder_dsrc_v2_ivim_pdu_descriptions::WerDecoder* decoder_;
    };
}  // Closing namespace wind_ros
}  // Closing namespace wind

#endif //DSRC_V2_IVIM_PDU_DESCRIPTIONS_RECEIVER_H