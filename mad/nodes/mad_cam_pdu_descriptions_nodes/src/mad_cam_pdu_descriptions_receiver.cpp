...
{	
    ...
    if(messageId == 2 && protocolVersion == 2) {
        ...
        slogger_->print() << "Incoming MADCAM: counter(" << ++messages_counter_ << ")";
        ...
        if(decoded)
        {
            slogger_->print() << "Incoming MADCAM: msgId("      << messageId   << ":" << protocolVersion << 
			                    ") stationId("      << stationId << 
                                ") test("           << bit0      << 
                                ") mirror("         << bit1      << 
                                ")";
            if(bit1) {
                slogger_->print() << "Publishing MADCAM on topic " << SENDER_ROS_TOPIC;
                mirror_publisher_->publish(msg);
            } else {
                slogger_->print() << "Publishing MADCAM on topic " << MAGENTA << topic_ << RESET;
