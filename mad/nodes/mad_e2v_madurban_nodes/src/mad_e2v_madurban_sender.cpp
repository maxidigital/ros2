 ...
 *
 */
#include <mad_e2v_madurban_sender.h>
 ...
 */
void
wind::wind_ros::Sender_mad_e2v_madurban::start()
{
    encoder_ = new wind::encoder_mad_e2v_madurban::WerEncoder(slogger_, debug_);
	...
	this->udp_sender_ = new wind::comm::UDPSender(output_ip_, port.str());
		...
		this->subscriber_ = node_handle_.subscribe(topic_, 10, &wind::wind_ros::Sender_mad_e2v_madurban::callback, this);
	#else		
		auto callback1 = [this](const mad_e2v_madurban_msgs::msg::EPUtoVehicle::SharedPtr msg) {this->callback(msg);};
		this->subscriber = this->create_subscription<mad_e2v_madurban_msgs::msg::EPUtoVehicle>(topic_, 10, callback1);
    #endif
 ...
 */
void
wind::wind_ros::Sender_mad_e2v_madurban::callback(
#if WIND_ROS_VERSION == 1
  const mad_e2v_madurban_msgs::EPUtoVehicle::ConstPtr& msg)
#else		
  const std::shared_ptr<mad_e2v_madurban_msgs::msg::EPUtoVehicle>& msg)
#endif
{	
	if(busy_)
		return;
    ...
    slogger_->print() << "EPUtoVehicle message received on topic '" << tools::green(topic_) << "'";
    ...
    //slogger_->print() << "EPUtoVehicle REQUEST messages sent: success(" << ++messages_counter_ << "/" << failed_ << ")";
...
void
wind::wind_ros::Sender_mad_e2v_madurban::print_message(std::size_t headerLen, std::size_t messageLen) {
	...
	slogger_->print() << "Sending message: name(EPUtoVehicle) header(REQUEST) " << _host_.str() << " " << _len_.str() << _success_;	
}
